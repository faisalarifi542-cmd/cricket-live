...
function Detail() {
  ...
  const { data, loading, reload } = useResource(() => teamsApi.get(id), [id]);
  ...
  return (
    <>
      <PageHeader
        title={String(team.name ?? `Team ${id}`)}
        description={`Team ID ${id}`}
        right={
          <>
            <Button variant="ghost" icon={<ChevronLeft className="h-4 w-4" />} onClick={() => router.push('/teams')}>
              Back
            </Button>
