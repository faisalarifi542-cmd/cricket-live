...
function PlayerFormDialog({
  ...
  useEffect(() => {
    ...
    setFetchId(player && player !== 'new' ? (player.external_id ?? player.player_external_id ?? '') : '');
  }, [player]);
  ...
  async function fetchFromProvider() {
    ...
    if (!id) {
      toast.error('Enter a provider player ID first');
      return;
    }
    setFetching(true);
    try {
      const res = await playersApi.fetchByProviderId(id);
      const d = res.data || {};
      // Auto-fill only the fields the provider returned; keep anything the
      // operator already typed (and never clobber an uploaded admin image).
      patch({
        name: d.name || d.full_name || form.name,
        full_name: d.full_name || d.name || form.full_name,
        country: d.country || form.country,
        role: d.role || form.role,
        batting_style: d.batting_style || form.batting_style,
        bowling_style: d.bowling_style || form.bowling_style,
        external_id: d.external_id || id,
        player_external_id: d.player_external_id || d.external_id || id,
        provider_image_url: d.provider_image_url || form.provider_image_url,
      });
  ...
  return (
    ...
    >
      <div className="mb-4 rounded-xl border border-cyan-500/20 bg-cyan-500/[0.04] p-4">
        <Field label="Fetch player from provider by ID" hint="Type the Cricbuzz player ID and click Fetch to auto-fill the form. You can edit everything before saving.">
          <div className="flex gap-2">
            <Input
              value={fetchId}
