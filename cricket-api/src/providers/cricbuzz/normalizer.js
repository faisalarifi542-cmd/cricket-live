...
export function normalizeMatchDetail(raw) {
  ...
  // IMPORTANT: the matchHeader teams carry the team logo `imageId` even for a
  // live match, while the miniscore `matchTeamInfo` block does NOT. The old code
  // REPLACED the header team with the miniscore one whenever the header lacked a
  // teamId, which dropped `imageId` and made the Live Player fall back to plain
  // initials for teams that have no local flag asset (e.g. franchise sides like
  // SFU/LAKR), even though Home showed the real logo. We now MERGE: take the
  // miniscore ids/names only to fill gaps, and always preserve the header
  // team's imageId/logo so the resolved logo URL survives into the detail.
  ...
  if (matchTeamInfo.length >= 1 && (!team1Raw?.teamId && !team1Raw?.id)) {
    ...
    team1Raw = {
      ...(team1Raw || {}),
      teamId: team1Raw?.teamId || team1Raw?.id || info.battingTeamId,
      teamSName: team1Raw?.teamSName || team1Raw?.shortName || info.battingTeamShortName,
      teamName: team1Raw?.teamName || team1Raw?.name || info.battingTeamName || info.battingTeamShortName,
    };
    team2Raw = {
      ...(team2Raw || {}),
      teamId: team2Raw?.teamId || team2Raw?.id || info.bowlingTeamId,
      teamSName: team2Raw?.teamSName || team2Raw?.shortName || info.bowlingTeamShortName,
      teamName: team2Raw?.teamName || team2Raw?.name || info.bowlingTeamName || info.bowlingTeamShortName,
    };
  }
