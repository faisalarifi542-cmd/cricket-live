...
import { getTeamLogoOrder, isTeamLogosEnabled, DEFAULT_TEAM_LOGO_ORDER } from './team-logos.js';
...
export async function buildPublicAppConfig() {
  ...
  ads.frequency = {
    ...
    // Master pre-roll switch. Default ON so existing configs are unchanged.
    preRollEnabled: normalizeBool(adsValue?.pre_roll_enabled, true),
    // Max pre-rolls that may show within one match/live-player screen
    // (repeated quality/server switches). 0 = no per-match limit. Default 1.
    preRollMaxPerMatch: toNumber(adsValue?.pre_roll_max_per_match, 1),
    // How long the app waits for the whole pre-roll attempt before opening the
    // stream anyway (fail-open). Default 8s; client clamps to a sane range.
    preRollTimeoutSeconds: toNumber(adsValue?.pre_roll_timeout_seconds, 8),
    ...ads.frequencyConfig,
  };
  ...
  // Global team-logo source priority order + master enable flag. The backend
  // already resolves the admin/api logo server-side; exposing the order lets the
  // app place the client-only `local` source at the configured position and fall
  // back to initials exactly as the admin intends.
  let teamLogoOrder = DEFAULT_TEAM_LOGO_ORDER.slice();
  let teamLogosEnabled = true;
  try {
    [teamLogoOrder, teamLogosEnabled] = await Promise.all([
      getTeamLogoOrder(),
      isTeamLogosEnabled(),
    ]);
  } catch {
    teamLogoOrder = DEFAULT_TEAM_LOGO_ORDER.slice();
    teamLogosEnabled = true;
  }
  ...
  return {
    ...
    playerImageMode,
    // Surfaced at top level AND under `team` so older/newer clients both find it.
    // The app reads `teamLogoOrder` / `teamLogosEnabled`.
    teamLogoOrder,
    teamLogosEnabled,
    ads,
    notifications,
    ...
    team: { logoOrder: teamLogoOrder, logosEnabled: teamLogosEnabled },
    app,
