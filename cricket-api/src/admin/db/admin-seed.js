...
import { ensureDefaultProvider } from '../provider-seed.js';
...
async function syncRolePermissions(roleSlug, permissionSlugs) {
  ...
  const slugs = permissionSlugs.length ? permissionSlugs : ['__none__'];
  const placeholders = slugs.map(() => '?').join(', ');
  const perms = await query(
    `SELECT id, slug FROM admin_permissions WHERE slug IN (${placeholders})`,
    slugs,
  );
