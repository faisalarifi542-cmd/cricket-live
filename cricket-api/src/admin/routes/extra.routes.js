...
import {
  ...
  coerceTeamLogoOrder,
  TEAM_LOGO_SOURCES,
  TEAM_LOGO_ORDER_SETTING_KEY,
  TEAM_LOGOS_ENABLED_SETTING_KEY,
...
export default async function extraAdminRoutes(fastify) {
  ...
  // Global team-logo source priority order + master enable flag. The order is
  // a permutation of ["admin","local","api","initials"]; `local` is enforced
  // client-side, the backend acts on admin/api/initials. Stored in app_settings.
  fastify.get('/teams/logo-order', { preHandler: [requirePermissions('teams.view')] }, async () => {
    const [order, enabled] = await Promise.all([getTeamLogoOrder(), isTeamLogosEnabled()]);
    return { success: true, order, enabled, sources: TEAM_LOGO_SOURCES };
  });

  fastify.put('/teams/logo-order', { preHandler: [requirePermissions('teams.write')] }, async (request, reply) => {
    const body = request.body || {};
    const order = coerceTeamLogoOrder(body.order);
    if (!Array.isArray(body.order) && typeof body.order !== 'string') {
      return reply.code(400).send({ success: false, error: 'order is required' });
    }
    const enabled = body.enabled == null ? true : Boolean(body.enabled);
    await withAudit(
      request,
      { action: 'teams.logoOrder.set', entityType: 'team', newValue: { order, enabled } },
      async () => {
        await query(
          `INSERT INTO app_settings (setting_key, setting_value, \`group\`, description, updated_by)
           VALUES (?, ?, 'teams', 'Team logo source priority order', ?)
           ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), updated_by = VALUES(updated_by)`,
          [TEAM_LOGO_ORDER_SETTING_KEY, JSON.stringify(order), request.adminUser?.id || null],
        );
        await query(
          `INSERT INTO app_settings (setting_key, setting_value, \`group\`, description, updated_by)
           VALUES (?, ?, 'teams', 'Master switch for team logos', ?)
           ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), updated_by = VALUES(updated_by)`,
          [TEAM_LOGOS_ENABLED_SETTING_KEY, JSON.stringify(enabled), request.adminUser?.id || null],
        );
      },
    );
    invalidateAdminTeamLogoIndex();
    await invalidatePublicConfigCaches();
    return { success: true, order, enabled };
  });
