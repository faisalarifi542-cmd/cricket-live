...
export default async function apiKeyRoutes(fastify) {
  ...
  fastify.post('/:id/revoke', { preHandler: [requirePermissions('apiKeys.write')] }, async (request, reply) => {
    const id = request.params.id;
    const old = await query(`SELECT id, name, is_active FROM api_keys WHERE id = ?`, [id]);
    if (!old.length) return reply.code(404).send({ success: false, error: 'Not found' });
    await withAudit(
      request,
      { action: 'apiKey.revoke', entityType: 'api_key', entityId: id, oldValue: old[0], newValue: { is_active: 0 } },
      async () => query(`UPDATE api_keys SET is_active = 0 WHERE id = ?`, [id]),
    );
    return { success: true };
  });
}
