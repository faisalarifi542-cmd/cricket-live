...
class CricketApiService {
  ...
  /// Lightweight live-score feed. Polls ONLY the score/status fields for the
  /// given match ids from the fast per-match backend endpoint (no heavy home
  /// aggregate, no series/images/streams). The response objects share the
  /// normalized match-detail shape, so [CricketMatch.fromJson] parses them
  /// directly — only score-bearing fields are populated. Returns an empty list
  /// for an empty id set without hitting the network.
  Future<ApiEnvelope<List<CricketMatch>>> liveScores(List<String> ids) async {
    final clean = ids.where((s) => s.trim().isNotEmpty).toList();
    if (clean.isEmpty) {
      return ApiEnvelope.fromJson(
          const {'data': <dynamic>[]}, (value) => <CricketMatch>[]);
    }
    final json = await _client.get('/app/live-scores', query: {
      'ids': clean.join(','),
    });
    return ApiEnvelope.fromJson(
  ...
  /// Admin-managed decorative assets (backgrounds/overlays) keyed by name +
  /// theme. `allowFailure` so older backends without this endpoint degrade to
  /// the app's bundled assets.
  Future<ApiEnvelope<Map<String, dynamic>>> appAssets() =>
      _map('/app/assets', allowFailure: true);
  ...
  /// FAST live commentary for one match from `/app/live-commentary` — the
  /// freshest provider source (Cricbuzz `/comm/{id}`), short server TTL +
  /// single-flight, mirroring [liveScores]. Returns `{ items: [...] }` for the
  /// requested id (the endpoint wraps per-id rows in `data:[{match_id,items}]`,
  /// so we unwrap the single row here). `allowFailure` so an older backend
  /// without the route degrades to the full-commentary feed at the caller.
  Future<ApiEnvelope<Map<String, dynamic>>> liveCommentary(
      String matchId) async {
    if (matchId.trim().isEmpty) {
      return ApiEnvelope.fromJson(
          const {'data': <String, dynamic>{}}, (v) => <String, dynamic>{});
    }
    final json = await _client.get('/app/live-commentary',
        query: {'ids': matchId}, allowFailure: true);
    return ApiEnvelope.fromJson(json, (value) {
      ...
      final row = rows.isEmpty
          ...
          : (rows.first is Map
              ...
              : const <String, dynamic>{});
      return <String, dynamic>{
        'items': _asList(row['items']),
        'source': row['source'] ?? 'full-commentary',
        'latestOver': row['latestOver'],
        'scoreOver': row['scoreOver'],
        'providerLag': row['providerLag'] == true,
        'completeHistory': row['completeHistory'] == true,
        'sourceCandidates': _asList(row['sourceCandidates']),
