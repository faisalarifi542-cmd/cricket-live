...
/// Resolves a generic REMOTE image URL (admin/provider/Cricbuzz) from an image
/// id or logo/image fields. This is the low-level resolver shared by news,
/// series and team parsers. It does NOT apply the team-logo source order, inject
/// local assets, or gate on the team-logos setting — for team logos use
/// [resolveTeamLogoUrl], which layers that ordering on top of this.
String? resolveCricbuzzImageUrl(dynamic value) {
  ...
  // Prefer an explicit admin logo so it is never lost even when a stale provider
  // URL also sits on the node.
...
/// Resolves the TEAM logo to display, honoring the admin-configured source
/// priority order (see [TeamLogoResolver]). The backend already resolves the
/// admin-vs-api remote logo server-side and reports its choice via `logoSource`;
/// this inserts the client-only bundled `local` flag asset at the configured
/// position and applies an `initials`-first / disabled choice.
///
/// Default order is `admin → local → api → initials`, so an admin logo always
/// wins, then a bundled flag, then the provider logo, then initials. Returns an
/// http URL, a local asset path, or null (→ render initials in the widget).
String? resolveTeamLogoUrl(dynamic value) {
  if (value is String) {
    final remote = resolveCricbuzzImageUrl(value);
    return TeamLogoResolver.resolve(serverRemote: remote, logoSource: 'api');
  }
  final json = apiMap(value);
  final imageDetails = apiMap(json['imageDetails'] ?? json['image_details']);
  // Trust the backend's already order-resolved `logo_url`/`logoUrl` as the
  // remote (it applied the admin/api/initials order server-side). `adminLogoUrl`
  // is used only to LABEL the source, never to override that choice.
  var serverRemote = resolveCricbuzzImageUrlFromFields(
  ...
  final adminUrl = apiString(json['adminLogoUrl'] ?? json['admin_logo_url']);
  // Fall back to the admin URL only when no resolved/provider remote exists
  // (e.g. a payload that wasn't enriched by the backend).
  if (serverRemote == null && adminUrl.isNotEmpty) serverRemote = adminUrl;
  var logoSource = apiString(json['logoSource'] ?? json['logo_source']);
  if (logoSource.isEmpty && serverRemote != null) {
    logoSource = (adminUrl.isNotEmpty && adminUrl == serverRemote) ? 'admin' : 'api';
  }

  // Client-only sources (never for players): bundled local flag asset, then a
  // known Cricbuzz image id as an api fallback when the payload carried none.
  ...
  if (!hasPlayerIdentity) {
    final id = json['team_id'] ?? json['teamId'] ?? json['teamID'] ?? json['id'];
    final name = json['teamName'] ??
        json['team_name'] ??
        json['fullName'] ??
        json['name'];
    final shortName = json['teamSName'] ??
        json['team_s_name'] ??
        json['teamShort'] ??
        json['team_short'] ??
        json['shortName'] ??
        ...
        json['abbr'] ??
        json['code'];
    localAsset = localTeamLogoAsset(id: id, name: name, shortName: shortName);
    if (serverRemote == null) {
      final knownApi =
          knownCricbuzzTeamLogoUrl(id: id, name: name, shortName: shortName);
      if (knownApi != null) {
        serverRemote = knownApi;
        if (logoSource.isEmpty) logoSource = 'api';
      }
    }
  }

  return TeamLogoResolver.resolve(
...
/// Resolves a player's face image, honoring the global admin player-image mode.
///
/// The backend already enriches every player object: it sets `imageUrl` /
/// `image_url` to the correct admin/provider photo (or null) for the active
/// mode, and clears raw provider id fields when no photo is allowed. This
/// function therefore ONLY consumes the server-resolved URL and gates it through
/// [PlayerImageResolver] — it never rebuilds a Cricbuzz URL from a raw
/// `imageId`/`faceImageId`, which is what previously let provider photos leak
/// into `admin_only` / `initials_only` modes (and the Rankings screen).
String? resolvePlayerImageUrl(dynamic value) {
  if (value is String) {
    final text = value.trim();
    // Only an already-built http(s) URL is trusted; a bare id is ignored so we
    // never reconstruct a provider photo the mode may forbid.
    final serverUrl = (text.startsWith('http://') || text.startsWith('https://'))
        ? text
        : null;
    return PlayerImageResolver.resolveServerImage(serverUrl);
  }
  final json = apiMap(value);
  final imageDetails = apiMap(json['imageDetails'] ?? json['image_details']);
  final playerId =
  ...
  final directImage = apiString(json['image_url'] ??
      ...
      imageDetails['url'] ??
      imageDetails['imageUrl']);
  // Guard against a stray team-logo URL keyed by the player's own id.
  final isHttp =
      directImage.startsWith('http://') || directImage.startsWith('https://');
  final looksLikeSelfLogo = isHttp &&
      playerId.isNotEmpty &&
      directImage.contains('/c$playerId/');
  final serverUrl = (isHttp && !looksLikeSelfLogo) ? directImage : null;
  final name = apiString(json['fullName'] ??
      json['full_name'] ??
      json['name'] ??
      json['playerName'] ??
      json['player_name']);
...
/// Resolves a local rounded flag asset (`assets/flags/rounded/<team>.webp`) for
...
/// Resolves ONLY a bundled local team logo asset (rounded flag, then other
/// bundled team logos). Returns an `assets/...` path or null. This is the
/// client-only `local` source in [TeamLogoResolver]; it never returns a remote
/// URL, so the resolver can position `local` independently of `api`.
String? localTeamLogoAsset({dynamic id, dynamic name, dynamic shortName}) {
...
/// Resolves a known Cricbuzz team-image URL for a famous national side from its
/// hardcoded image id. Returns an http URL or null. This is an `api`-source
/// fallback used only when the backend payload carried no image id of its own.
String? knownCricbuzzTeamLogoUrl({dynamic id, dynamic name, dynamic shortName}) {
  for (final raw in [apiString(id), apiString(shortName), apiString(name)]) {
    final key = _normalizeTeamKey(raw);
    if (key.isEmpty) continue;
...
/// Back-compat combiner: local asset first, then a known Cricbuzz image URL.
/// Prefer [localTeamLogoAsset] / [knownCricbuzzTeamLogoUrl] for order-aware code.
String? resolveKnownTeamLogoUrl({
  dynamic id,
  dynamic name,
  dynamic shortName,
}) {
  return localTeamLogoAsset(id: id, name: name, shortName: shortName) ??
      knownCricbuzzTeamLogoUrl(id: id, name: name, shortName: shortName);
}
...
const Map<String, String> _knownTeamLogoAssets = {
  '13': 'assets/images/team_nz.webp',
  'NZ': 'assets/images/team_nz.webp',
  'NEW ZEALAND': 'assets/images/team_nz.webp',
  '10': 'assets/images/team_wi.webp',
  'WI': 'assets/images/team_wi.webp',
  'WEST INDIES': 'assets/images/team_wi.webp',
  '11': 'assets/images/team_sa.webp',
  'SA': 'assets/images/team_sa.webp',
  'SOUTH AFRICA': 'assets/images/team_sa.webp',
  '6': 'assets/images/team_ban.webp',
  'BAN': 'assets/images/team_ban.webp',
  'BANGLADESH': 'assets/images/team_ban.webp',
};
...
const Map<String, String> kRoundedFlagAssets = {
  'AFG': 'assets/flags/rounded/afghanistan.webp',
  'AFGHANISTAN': 'assets/flags/rounded/afghanistan.webp',
  'ARG': 'assets/flags/rounded/argentina.webp',
  'ARGENTINA': 'assets/flags/rounded/argentina.webp',
  'AUS': 'assets/flags/rounded/australia.webp',
  'AUSTRALIA': 'assets/flags/rounded/australia.webp',
  'AUSTRIA': 'assets/flags/rounded/austria.webp',
  'AUT': 'assets/flags/rounded/austria.webp',
  'BAH': 'assets/flags/rounded/bahamas.webp',
  'BAHAMAS': 'assets/flags/rounded/bahamas.webp',
  'BAHRAIN': 'assets/flags/rounded/bahrain.webp',
  'BAN': 'assets/flags/rounded/bangladesh.webp',
  'BANGLADESH': 'assets/flags/rounded/bangladesh.webp',
  'BEL': 'assets/flags/rounded/belgium.webp',
  'BELGIUM': 'assets/flags/rounded/belgium.webp',
  'BELIZE': 'assets/flags/rounded/belize.webp',
  'BER': 'assets/flags/rounded/bermuda.webp',
  'BERMUDA': 'assets/flags/rounded/bermuda.webp',
  'BHR': 'assets/flags/rounded/bahrain.webp',
  'BIZ': 'assets/flags/rounded/belize.webp',
  'BOT': 'assets/flags/rounded/botswana.webp',
  'BOTSWANA': 'assets/flags/rounded/botswana.webp',
  'BRA': 'assets/flags/rounded/brazil.webp',
  'BRAZIL': 'assets/flags/rounded/brazil.webp',
  'CAN': 'assets/flags/rounded/canada.webp',
  'CANADA': 'assets/flags/rounded/canada.webp',
  'CAY': 'assets/flags/rounded/cayman_islands.webp',
  'CAYMAN ISLANDS': 'assets/flags/rounded/cayman_islands.webp',
  'CHI': 'assets/flags/rounded/chile.webp',
  'CHILE': 'assets/flags/rounded/chile.webp',
  'CHINA': 'assets/flags/rounded/china.webp',
  'CHN': 'assets/flags/rounded/china.webp',
  'COK': 'assets/flags/rounded/cook_islands.webp',
  'COOK ISLANDS': 'assets/flags/rounded/cook_islands.webp',
  'CZE': 'assets/flags/rounded/czech_republic.webp',
  'CZECH REPUBLIC': 'assets/flags/rounded/czech_republic.webp',
  'DEN': 'assets/flags/rounded/denmark.webp',
  'DENMARK': 'assets/flags/rounded/denmark.webp',
  'ENG': 'assets/flags/rounded/england.webp',
  'ENGLAND': 'assets/flags/rounded/england.webp',
  'ESP': 'assets/flags/rounded/spain.webp',
  'FIJ': 'assets/flags/rounded/fiji.webp',
  'FIJI': 'assets/flags/rounded/fiji.webp',
  'FIN': 'assets/flags/rounded/finland.webp',
  'FINLAND': 'assets/flags/rounded/finland.webp',
  'FRA': 'assets/flags/rounded/france.webp',
  'FRANCE': 'assets/flags/rounded/france.webp',
  'GER': 'assets/flags/rounded/germany.webp',
  'GERMANY': 'assets/flags/rounded/germany.webp',
  'GHA': 'assets/flags/rounded/ghana.webp',
  'GHANA': 'assets/flags/rounded/ghana.webp',
  'GIB': 'assets/flags/rounded/gibraltar.webp',
  'GIBRALTAR': 'assets/flags/rounded/gibraltar.webp',
  'GSY': 'assets/flags/rounded/guernsey.webp',
  'GUERNSEY': 'assets/flags/rounded/guernsey.webp',
  'HK': 'assets/flags/rounded/hong_kong.webp',
  'HKG': 'assets/flags/rounded/hong_kong.webp',
  'HONG KONG': 'assets/flags/rounded/hong_kong.webp',
  'HUN': 'assets/flags/rounded/hungary.webp',
  'HUNGARY': 'assets/flags/rounded/hungary.webp',
  'INA': 'assets/flags/rounded/indonesia.webp',
  'IND': 'assets/flags/rounded/india.webp',
  'INDIA': 'assets/flags/rounded/india.webp',
  'INDONESIA': 'assets/flags/rounded/indonesia.webp',
  'IOM': 'assets/flags/rounded/isle_of_man.webp',
  'IRE': 'assets/flags/rounded/ireland.webp',
  'IRELAND': 'assets/flags/rounded/ireland.webp',
  'ISLE OF MAN': 'assets/flags/rounded/isle_of_man.webp',
  'ITA': 'assets/flags/rounded/italy.webp',
  'ITALY': 'assets/flags/rounded/italy.webp',
  'JAPAN': 'assets/flags/rounded/japan.webp',
  'JER': 'assets/flags/rounded/jersey.webp',
  'JERSEY': 'assets/flags/rounded/jersey.webp',
  'JPN': 'assets/flags/rounded/japan.webp',
  'KEN': 'assets/flags/rounded/kenya.webp',
  'KENYA': 'assets/flags/rounded/kenya.webp',
  'KSA': 'assets/flags/rounded/saudi_arabia.webp',
  'KUW': 'assets/flags/rounded/kuwait.webp',
  'KUWAIT': 'assets/flags/rounded/kuwait.webp',
  'MALAWI': 'assets/flags/rounded/malawi.webp',
  'MALAYSIA': 'assets/flags/rounded/malaysia.webp',
  'MALTA': 'assets/flags/rounded/malta.webp',
  'MAS': 'assets/flags/rounded/malaysia.webp',
  'MEX': 'assets/flags/rounded/mexico.webp',
  'MEXICO': 'assets/flags/rounded/mexico.webp',
  'MLT': 'assets/flags/rounded/malta.webp',
  'MWI': 'assets/flags/rounded/malawi.webp',
  'NAM': 'assets/flags/rounded/namibia.webp',
  'NAMIBIA': 'assets/flags/rounded/namibia.webp',
  'NED': 'assets/flags/rounded/netherlands.webp',
  'NEP': 'assets/flags/rounded/nepal.webp',
  'NEPAL': 'assets/flags/rounded/nepal.webp',
  'NETHERLANDS': 'assets/flags/rounded/netherlands.webp',
  'NEW ZEALAND': 'assets/flags/rounded/new_zealand.webp',
  'NGA': 'assets/flags/rounded/nigeria.webp',
  'NIGERIA': 'assets/flags/rounded/nigeria.webp',
  'NOR': 'assets/flags/rounded/norway.webp',
  'NORWAY': 'assets/flags/rounded/norway.webp',
  'NZ': 'assets/flags/rounded/new_zealand.webp',
  'OMA': 'assets/flags/rounded/oman.webp',
  'OMAN': 'assets/flags/rounded/oman.webp',
  'PAK': 'assets/flags/rounded/pakistan.webp',
  'PAKISTAN': 'assets/flags/rounded/pakistan.webp',
  'PAN': 'assets/flags/rounded/panama.webp',
  'PANAMA': 'assets/flags/rounded/panama.webp',
  'PAPUA NEW GUINEA': 'assets/flags/rounded/papua_new_guinea.webp',
  'PER': 'assets/flags/rounded/peru.webp',
  'PERU': 'assets/flags/rounded/peru.webp',
  'PHI': 'assets/flags/rounded/philippines.webp',
  'PHILIPPINES': 'assets/flags/rounded/philippines.webp',
  'PNG': 'assets/flags/rounded/papua_new_guinea.webp',
  'POR': 'assets/flags/rounded/portugal.webp',
  'PORTUGAL': 'assets/flags/rounded/portugal.webp',
  'QAT': 'assets/flags/rounded/qatar.webp',
  'QATAR': 'assets/flags/rounded/qatar.webp',
  'ROMANIA': 'assets/flags/rounded/romania.webp',
  'ROU': 'assets/flags/rounded/romania.webp',
  'RSA': 'assets/flags/rounded/south_africa.webp',
  'RWA': 'assets/flags/rounded/rwanda.webp',
  'RWANDA': 'assets/flags/rounded/rwanda.webp',
  'SA': 'assets/flags/rounded/south_africa.webp',
  'SAM': 'assets/flags/rounded/samoa.webp',
  'SAMOA': 'assets/flags/rounded/samoa.webp',
  'SAUDI ARABIA': 'assets/flags/rounded/saudi_arabia.webp',
  'SCO': 'assets/flags/rounded/scotland.webp',
  'SCOT': 'assets/flags/rounded/scotland.webp',
  'SCOTLAND': 'assets/flags/rounded/scotland.webp',
  'SIERRA LEONE': 'assets/flags/rounded/sierra_leone.webp',
  'SIN': 'assets/flags/rounded/singapore.webp',
  'SINGAPORE': 'assets/flags/rounded/singapore.webp',
  'SL': 'assets/flags/rounded/sri_lanka.webp',
  'SLE': 'assets/flags/rounded/sierra_leone.webp',
  'SOUTH AFRICA': 'assets/flags/rounded/south_africa.webp',
  'SPAIN': 'assets/flags/rounded/spain.webp',
  'SRI LANKA': 'assets/flags/rounded/sri_lanka.webp',
  'SUI': 'assets/flags/rounded/switzerland.webp',
  'SWE': 'assets/flags/rounded/sweden.webp',
  'SWEDEN': 'assets/flags/rounded/sweden.webp',
  'SWITZERLAND': 'assets/flags/rounded/switzerland.webp',
  'TAN': 'assets/flags/rounded/tanzania.webp',
  'TANZANIA': 'assets/flags/rounded/tanzania.webp',
  'THA': 'assets/flags/rounded/thailand.webp',
  'THAILAND': 'assets/flags/rounded/thailand.webp',
  'U A E': 'assets/flags/rounded/uae.webp',
  'UAE': 'assets/flags/rounded/uae.webp',
  'UGA': 'assets/flags/rounded/uganda.webp',
  'UGANDA': 'assets/flags/rounded/uganda.webp',
  'UNITED ARAB EMIRATES': 'assets/flags/rounded/uae.webp',
  'UNITED STATES': 'assets/flags/rounded/united_states.webp',
  'UNITED STATES OF AMERICA': 'assets/flags/rounded/united_states.webp',
  'USA': 'assets/flags/rounded/united_states.webp',
  'VAN': 'assets/flags/rounded/vanuatu.webp',
  'VANUATU': 'assets/flags/rounded/vanuatu.webp',
  'WEST INDIES': 'assets/flags/rounded/west_indies.webp',
  'WI': 'assets/flags/rounded/west_indies.webp',
  'WINDIES': 'assets/flags/rounded/west_indies.webp',
  'ZIM': 'assets/flags/rounded/zimbabwe.webp',
  'ZIMBABWE': 'assets/flags/rounded/zimbabwe.webp',
};
...
class ApiTeam {
  ...
  factory ApiTeam.fromJson(dynamic value) {
    ...
    final logoUrl = resolveTeamLogoUrl(json);
...
class StreamSource {
  ...
  /// Raw admin/back-end type, lower-cased, with common HLS aliases collapsed to
  /// 'hls'. Covers content-type style values the backend may forward.
  String get type {
    final raw = apiString(streamType, 'hls').toLowerCase().trim();
    const hlsAliases = {
      'hls',
      'm3u8',
      'mpegurl',
    ...
    if (hlsAliases.contains(raw)) return 'hls';
    return raw;
  }

  /// True when the URL path (ignoring any query string / token) contains
  /// `.m3u8`. Authoritative HLS evidence regardless of the stored type, which
  /// may be missing/unknown/wrong.
  bool get _urlIsM3u8 {
    final path = (Uri.tryParse(url)?.path ?? url).toLowerCase();
    return path.contains('.m3u8');
  }

  bool get isHls => type == 'hls' || _urlIsM3u8;
  // A clear .m3u8 URL is HLS — never DASH/external, even if the type says so.
  bool get isDash =>
      !_urlIsM3u8 &&
      (type == 'dash' || type == 'mpd' || url.toLowerCase().contains('.mpd'));
  bool get isExternal =>
      !_urlIsM3u8 && (type == 'external' || type == 'iframe');
...
class RankingEntry {
  ...
  factory RankingEntry.fromJson(dynamic value) {
    ...
    return RankingEntry(
      ...
      // Player rankings must obey the global player-image mode (the screen used
      // to rebuild a Cricbuzz URL from a raw imageId, bypassing admin/initials
      // settings). Team rankings keep the team-logo resolver.
      imageUrl: isTeam
          ? resolveTeamLogoUrl(json)
          : resolvePlayerImageUrl(json),
      isTeam: isTeam,
    );
  }
}
...
class ApiPlayer {
  ...
  factory ApiPlayer.fromJson(dynamic value) {
    ...
    // Honor the global player-image mode: consume the server-resolved photo and
    // gate it (never rebuild a Cricbuzz URL from the raw imageId here).
    final resolved = resolvePlayerImageUrl(json);
    return ApiPlayer(
...
class AppConfig {
  ...
  /// Global player-image resolution mode set in the Admin Panel. One of:
  /// admin_first, cricbuzz_first, admin_only, cricbuzz_only, initials_only.
  /// Defaults to admin_first (the backend default) when absent.
  String get playerImageMode =>
      _stringPath(['playerImageMode']) ??
      _stringPath(['player', 'imageMode']) ??
      'admin_first';

  /// Admin-configured team-logo source priority order. A permutation of
  /// `admin`, `local`, `api`, `initials`. Defaults to admin → local → api →
  /// initials when absent. Consumed by [TeamLogoResolver].
  List<String> get teamLogoOrder {
    final raw = _rawPath(['teamLogoOrder']) ?? _rawPath(['team', 'logoOrder']);
    if (raw is List) {
      final out = raw
    ...
    return const ['admin', 'local', 'api', 'initials'];
  }

  /// Master switch for team logos. When false, the app shows initials only.
  bool get teamLogosEnabled {
    final raw = _rawPath(['teamLogosEnabled']) ?? _rawPath(['team', 'logosEnabled']);
    if (raw == null) return true;
    return apiBool(raw, true);
  }
  ...
  /// Returns the raw value at [path] (may be a List/Map/scalar) or null.
  dynamic _rawPath(List<String> path) {
    dynamic current = values;
    for (final key in path) {
      if (current is Map<String, dynamic>) {
        current = current[key];
