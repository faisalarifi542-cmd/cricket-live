...
class CricketMatch {
  ...
  factory CricketMatch.fromJson(dynamic value) {
    ...
    final t1Score =
        _formatTeamScore(score?['team1'] ?? score?['teamA'] ?? t1.innings);
    final t2Score =
        _formatTeamScore(score?['team2'] ?? score?['teamB'] ?? t2.innings);
    final start = _str(
      json,
  ...
  static _TeamData _team(Map<String, dynamic> json, List<String> keys) {
    for (final key in keys) {
      ...
      if (value is Map<String, dynamic>) {
        ...
        final logoUrl = resolveCricbuzzImageUrl(value);
  ...
  static String _formatTeamScore(dynamic innings) {
    ...
    for (final raw in innings) {
      ...
      final normalizedOvers = overs == null ? '' : normalizeOversText(overs);
      final oversText = normalizedOvers.isEmpty ? '' : ' ($normalizedOvers OV)';
